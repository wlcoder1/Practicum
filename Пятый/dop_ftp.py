...
def try_decorator(path_func):
    def wrapper(*path):
        try:
            ...
            return (f'Данный файл уже создан') 
        except PermissionError:
            return f"Отказано в разрешении"
    return wrapper
...
def get_file(path):
    ...
    try:
        ...
        returned = b'Неверный путь'+FAIL_FLAG
    except PermissionError:
        returned = b"Отказано в разрешении"+FAIL_FLAG
    else:
        returned =END_FLAG
    log_print("Уже отправлен" )
    return returned
